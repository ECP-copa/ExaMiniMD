...
/* ----------------------------------------------------------------------
   ...
   ---------------------------------------------------------------------- */
/*
void ForceSNAP::compute_optimized(int eflag, int vflag)
{
  ...
  if (do_load_balance > 0 &&
    ...
    if (ilistmask_max < list->inum) {
      memory->grow(ilistmask,list->inum,"ForceSnap::ilistmask");
...
inline int ForceSNAP::equal(double* x,double* y)
{
  double dist2 =
    (x[0]-y[0])*(x[0]-y[0]) +
    (x[1]-y[1])*(x[1]-y[1]) +
...
inline double ForceSNAP::dist2(double* x,double* y)
{
  return
    (x[0]-y[0])*(x[0]-y[0]) +
    (x[1]-y[1])*(x[1]-y[1]) +
...
void ForceSNAP::load_balance()
{
  ...
  if (comm->cutghostuser <
    ...
    Kokkos::abort("Communication cutoff too small for SNAP micro load balancing");
          ...
          if (get_tag >= 0) {
            ...
            if (atom->nlocal + atom->nghost >= ghostnumneigh_max) {
              ...
              memory->grow(ghostnumneigh,ghostnumneigh_max,
                           "ForceSnap::ghostnumneigh");
              memory->grow(ghostfirstneigh,ghostnumneigh_max,
                           "ForceSnap::ghostfirstneigh");
            }
...
void ForceSNAP::set_sna_to_shared(int snaid,int i)
{
...
void ForceSNAP::build_per_atom_arrays()
{
  ...
  if (i_max < count || i_neighmax < neighmax) {
    ...
    memory->create(i_rij,i_maxt,i_neighmax,3,"ForceSNAP::i_rij");
    memory->create(i_inside,i_maxt,i_neighmax,"ForceSNAP::i_inside");
    memory->create(i_wj,i_maxt,i_neighmax,"ForceSNAP::i_wj");
    memory->create(i_rcutij,i_maxt,i_neighmax,"ForceSNAP::i_rcutij");
    memory->create(i_ninside,i_maxt,"ForceSNAP::i_ninside");
    memory->create(i_pairs,i_maxt*i_neighmax,4,"ForceSNAP::i_pairs");
  }
...
------------------------------------------------------------------------- */
/*
void ForceSNAP::settings(int narg, char **arg)
{
  ...
  for (int i=0; i < narg; i++) {
    if (i+2>narg) Kokkos::abort("Illegal pair_style command");
    if (strcmp(arg[i],"nthreads")==0) {
      nthreads=force->inumeric(FLERR,arg[++i]);
#if defined(LMP_USER_OMP)
      Kokkos::abort("Must set number of threads via package omp command");
#else
	...
	schedule_user = 5;
      if (schedule_user == 0)
	Kokkos::abort("Illegal pair_style command");
      continue;
    }
    Kokkos::abort("Illegal pair_style command");
  }
...
------------------------------------------------------------------------- */
/*
void ForceSNAP::init_style()
{
  if (force->newton_pair == 0)
    Kokkos::abort("Force style SNAP requires newton pair on");
...
------------------------------------------------------------------------- */
/*
double ForceSNAP::init_one(int i, int j)
{
  if (setflag[i][j] == 0) Kokkos::abort("All pair coeffs are not set");
...
------------------------------------------------------------------------- */
/*
double ForceSNAP::memory_usage()
{
  double bytes = Force::memory_usage();
  int n = atom->ntypes+1;
